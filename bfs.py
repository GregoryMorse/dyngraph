...
def do_lex_bfs_opt(virtual_root, succ, pred, test_order=None):
  #eT, bfs_int, bfs_revint, bfs_level = do_lex_bfs(virtual_root, succ, pred, test_order)
  #parts = graph.PartitionRefinement()
  #for i in bfs_revint:
  #  parts.add(bfs_revint[i], None if i==1 or {z for z in pred[bfs_revint[i]] if z in bfs_int and bfs_int[z] < i} != {z for z in pred[bfs_revint[i-1]] if z in bfs_int and bfs_int[z] < i-1} else parts.find(bfs_revint[i-1]))
  ...
  while True: #not parts.isEmpty():
    ...
    priors = {}
    for y in succ[x]:
      if x == y or y in eT.pred and (eT.level[y] < eT.level[x] or eT.level[x] == eT.level[y] and (y in bfs_int or (eT.pred[y] == eT.pred[x] and x in parts.find(y).dataval.get_nodes()))): continue
      reachable.add(y)
      s = parts.find(y)
      if not s is None:
        if not y in eT.pred: eT.add_node(y); eT.add_edge(x, y)
    ...
    if not parts.isIsolatedItem(x):
      presplit = {z for z in s.dataval.get_nodes() if z != x and z in bfs_int}
      ...
      postsplit = {z for z in s.dataval.get_nodes() if z != x and not z in bfs_int}
  ...
  for i in range(2, len(bfs_revint)+1):
    assert ({z for z in pred[bfs_revint[i]] if z in bfs_int and bfs_int[z] < i} == {z for z in pred[bfs_revint[i-1]] if z in bfs_int and bfs_int[z] < i-1}) == (parts.find(bfs_revint[i]) is parts.find(bfs_revint[i-1]))
...
def do_inc_add_edge_bfs_opt_nolex(succ, bfs_tree, bfs_int, bfs_revint, bfs_level, x, y):
  if not x in bfs_tree.level: return
  newnode = not y in bfs_tree.level
  ...
  lvl = bfs_tree.level[x]+(1 if bfs_tree.level[y] != bfs_tree.level[x] else 0)
  start = 1 if x == bfs_tree.virtual_root else bfs_level[lvl-1-1]+1
  end = bfs_level[lvl-1]
  p = end + 1
  thisLevel = x: {y}
  ...
  while True:
    ...
    while start <= end:
      ...
      e = bfs_tree.succ[node] if start != 1 else bfs_tree.root_succ
      i = len(e)
      if not node in thisLevel and (i == 0 or bfs_int[e[0]] == p): p += i
      else:
        it = itertools.chain(e, thisLevel[node]) if node in thisLevel else e 
        #print(node, e, thisLevel[node] if node in thisLevel else [])
        for q in it:
          #print(q)
          changelvl, isnew = node in thisLevel and q in thisLevel[node], not q in bfs_tree.pred
